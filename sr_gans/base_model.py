...
class BaseSRGAN(torch.nn.Module):
    ...
    def forward(self, lr_imgs: torch.Tensor) -> torch.Tensor:
        ...
        hr_imgs = torch.sigmoid(self.gen_model(lr_imgs))
        return hr_imgs
    ...
    def load_checkpoint(self, filename: str, lr: Optional[float] = None) -> None:
        """
        ...
        lr (Optional[float]): A new learning rate for the optimizers if you want to 
            change it after some time.
        """
    ...
    def fit(
            ...
            num_epochs: int, patience: int, lr: Optional[float] = None,
            checkpoint_filename: Optional[str] = None) -> None:
        """
        Perform the training and validation procedures. Use also patience for early
        stopping.
        ...
        val_dl (torch.utils.data.DataLoader): The Pytorch data loader for the 
            validation dataset.

        num_epochs (int): Maximum number of iterations on the training dataset.

        patience (int): Maximum number of tollerated iterations on the training dataset 
            where the GAN doesn't improve.
        ...
        if checkpoint_filename is not None:
            ...
            self.load_checkpoint(checkpoint_filename, lr=lr)
        ...
        for epoch in range(num_epochs):
            ...
            for i, batch in enumerate(train_dl):
                ...
                # check also for divergence
                gen_loss = gen_losses["loss"]
                if gen_loss.item() > 500.:

                    # log results
                    print(
                        f"Train Skipped [epoch {epoch + 1:3d}/{num_epochs:3d}] "
            ...
            for metric_name, metric in patience_metrics.items():
                print(f"[{metric_name} {metric:.2f}] ", end="")
            print("\n")
    ...
    def test(
            ...
            checkpoint_filename: Optional[str] = None) -> None:
        """
        Perform the testing procedures.

        Parameters:
        ===========
        test_dl (torch.utils.data.DataLoader): The Pytorch data loader for the
            testing dataset.
        ...
        test_metrics: Dict[str, List[float]] = dict()
        ...
        for i, batch in enumerate(test_dl):
            ...
            for metric_name, metric in gan_metrics.items():
                if metric_name in test_metrics:
                    test_metrics[metric_name].append(metric.item())
                else:
                    test_metrics[metric_name] = [metric.item()]
        ...
        for metric_name in test_metrics.keys():
            test_metrics[metric_name] = torch.mean(torch.tensor(
                test_metrics[metric_name])).item()
    ...
    @torch.no_grad()
    def predict(self, lr_img: torch.Tensor) -> torch.Tensor:
        lr_img = lr_img.unsqueeze(dim=0)
        gen_hr_img = torch.sigmoid(self.gen_model(lr_img))
        gen_hr_img = gen_hr_img.squeeze_()
        return gen_hr_img
